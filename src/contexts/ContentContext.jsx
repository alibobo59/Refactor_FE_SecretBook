...
Building a personal library is more than just collecting books—it's about creating a curated space that reflects your intellectual journey...`,
      ...
      },
      {
        ...
        slug: 'interview-bestselling-author-jane-doe',
        excerpt: 'We sit down with the acclaimed author to discuss her latest novel...',
        content: `# Interview with Bestselling Author Jane Doe

We had the pleasure of speaking with Jane Doe about her writing process, inspiration, and latest work...`,
        author: 'Emily Rodriguez',
        authorId: 3,
        category: 'Author Interviews',
        tags: ['interview', 'author', 'writing'],
        status: 'draft',
        publishedAt: null,
    ...
    const mockFaqs = [
      ...
      },
      {
        ...
        answer: 'We accept all major credit cards, PayPal, and cash on delivery for eligible locations.',
        category: 'Payment',
        order: 5,
        status: 'published',
        createdAt: '2024-01-01T10:00:00Z',
      ...
      },
      {
        ...
        answer: 'Orders can be cancelled within 1 hour of placement. After that, please contact customer service for assistance.',
        category: 'Orders',
        order: 6,
        status: 'draft',
        createdAt: '2024-01-01T10:00:00Z',
  ...
  const unpublishBlogPost = async (id) => {
    setBlogPosts(prev => prev.map(post => 
      post.id === id ? { 
        ...post, 
        status: 'draft',
  ...
  const reorderFaqs = async (newOrder) => {
    const reorderedFaqs = newOrder.map((id, index) => {
  ...
  const markFaqHelpful = async (id, helpful = true) => {
    setFaqs(prev => prev.map(faq => 
      faq.id === id ? { 
        ...faq, 
